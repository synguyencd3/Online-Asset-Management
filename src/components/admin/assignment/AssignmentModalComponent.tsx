...
import { getOneAssignemnt, getOneAssignmentUrl, } from "../../../services/AssignmentService";
...
export const AssignmentModelComponent = (props: Props) => {
	...
	const { data: assignmentData, isLoading } = useSWR<AssignmentModalModel>(
		props.data && props.data.id ? getOneAssignmentUrl(props.data.id) : null,
		getOneAssignemnt,
		{
			onError: (error) => {
				message.error(error.response? error.response.data.message : "Failed To Get Assignment");
				props.onHide();
			},
		}
	);
	...
	const assignment: AssignmentModalModel | undefined = assignmentData ? formatData(assignmentData) : undefined;

	return (
		<Modal {...props} dialogClassName="assignment-detail-modal" centered backdrop={false}>
			<Modal.Header>
					...
					<Container style={{ maxWidth: "95%" }}>
						{headers.map((header, index) => {
							if (assignment) {
								...
								const isLongerThan60 = assignment[header.value].toString().length > 60;
								if (isLongerThan60 && !isShowFull[index]) {
									value = assignment[header.value].toString().substring(0, 60);
								}
								return (
									<Row className="my-3 justify-content-between" key={header.name} id={"assignment_model_row_" + header.value}
									>
										<Col sm={3} name="header_title" className="">
											{header.name}
										</Col>
										<Col sm={9} id={"assignment_model_row_data_" + header.value} style={{ overflow: "auto" }}>
											{value}
											<span
												id={"show_more_" + header.value}
												onClick={() => { isShowFull[index] = !isShowFull[index]; setIsShowFull(() => [...isShowFull]); }}
												className="show-more"
												style={{ color: "red" }}
											>
												{isLongerThan60 ? !isShowFull[index] ? " Show more" : " Show less" : ""}
											</span>
										</Col>
									</Row>
								);
